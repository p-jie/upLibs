package p_jie.library.view.imageselect.ui;
...
import p_jie.library.R;
import p_jie.library.view.imageselect.DataHolder;
import p_jie.library.view.imageselect.ImageDataSource;
import p_jie.library.view.imageselect.ImagePicker;
import p_jie.library.view.imageselect.adapter.ImageFolderAdapter;
import p_jie.library.view.imageselect.adapter.ImageGridAdapter;
import p_jie.library.view.imageselect.bean.ImageFolder;
import p_jie.library.view.imageselect.bean.ImageItem;
import p_jie.library.view.imageselect.view.FolderPopUpWindow;
...
public class ImageGridActivity extends ImageBaseActivity implements ImageDataSource.OnImagesLoadedListener, ImageGridAdapter.OnImageItemClickListener, ImagePicker.OnImageSelectedListener, View.OnClickListener {
    ...
    public static final int REQUEST_PERMISSION_COMPRESS = 0x03;
    public static final int REQUEST_PERMISSION_SUCCESS = 0x04;
    ...
    private ProgressDialog progressDialog;  //压缩文件时显示的进度条
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.btn_ok) {
            //
            progressDialog = ProgressDialog.show(ImageGridActivity.this, "", "正在压缩图片...", false);

            handler.sendEmptyMessage(REQUEST_PERMISSION_COMPRESS);
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case REQUEST_PERMISSION_COMPRESS:
                    for (int i = 0; i < imagePicker.getSelectedImages().size(); i++) {
                        File tempFile = CompressHelper.getDefault(ImageGridActivity.this).compressToFile(FileUtil.getFileByPath(imagePicker.getSelectedImages().get(i).path));
                        ImageItem item = imagePicker.getSelectedImages().get(i);
                        item.path = tempFile.getAbsolutePath();
                        imagePicker.getSelectedImages().remove(i);
                        imagePicker.getSelectedImages().add(i, item);
                    }
                    handler.sendEmptyMessage(REQUEST_PERMISSION_SUCCESS);
                    break;
